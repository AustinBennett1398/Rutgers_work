...
String sql = "SELECT SUM(maxBid) as totalEarnings FROM (SELECT max(bid_amt) as maxBid FROM bid NATURAL JOIN auction WHERE bid.auction_num = auction.auction_num AND current_timestamp() > auction.end_date GROUP BY auction_num) as t1"; 
rs = st.executeQuery(sql);
rs.next();

Double total_earnings = rs.getDouble("totalEarnings");

out.println("Total earnings to date across all users (only auctions that have ended): " + total_earnings);
