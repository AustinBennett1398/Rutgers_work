...
int validPoint(int**, int, int, int, int);
...
int validMatrix(int** puzzle, int n, int row, int col, int numToCheck)
{
	/*int sum = 0;
	
	for(i = ((row/3)*3); i <= ((row/3)*3)+2; i++)
		for(j = ((col/3)*3); j <= ((col/3)*3)+2; j++)
			if(validPoint(puzzle, n, row, col, numToCheck) == 1)
				sum++;
...
int validPoint(int** puzzle,int n ,int row, int col, int numToCheck)
{
	int i,j;
	...
	{		
		...
		else
		{
			for(i = 3; i < 5; i++)
			{
				for(j = 3; j < 5; j++)
				{
					if(validPoint(puzzle, n, i, j, x) == 1)
					{
						validNum = x;
						validSum++;
					}
		...
		for(i = 0; i < n; i++)
			for(j = 0; j < n; j++)
			{
				...
				{		
					if(validPoint(puzzle, n, i, j, x) == 1)
					{
						for(i = (i/3)*3; i < ((i/3)*3)+2; i++)
						{
							for(j = (j/3)*3; j < ((j/3)*3)+2; j++)
							{
								if(validPoint(puzzle, n, i, j, x) == 1)
								{
									validNum = x;
